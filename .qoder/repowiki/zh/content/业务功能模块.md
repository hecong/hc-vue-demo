...
## 项目结构
前端采用Vue 3 + TypeScript + Element Plus，路由基于vue-router按需加载页面；API通过统一的请求封装层进行HTTP调用；各业务模块以独立视图组件呈现，配合API模块完成数据交互。
...
## 核心组件
- 路由与鉴权：全局前置守卫校验token与页面所需权限，动态设置标题，未登录跳转登录页，无权限跳转首页。
- 请求封装：统一设置Authorization头、拦截401自动登出、统一错误提示与状态码处理。
- 视图组件：每个业务模块以独立页面组件实现，包含表格、分页、表单、弹窗等交互元素。
...
## 架构总览
系统采用“路由驱动视图 + 统一API调用”的前后端分离架构。视图组件通过API模块发起请求，请求经统一拦截器处理，返回数据在组件内渲染为表格、表单、分页等UI。
...
- [src/api/system.ts:33-35](file://src/api/system.ts#L33-L35)
...
## 结论
HC管理系统通过清晰的路由与权限控制、统一的请求封装与API模块化设计，实现了用户管理、企业管理、角色权限、日志与个人中心等核心业务模块。视图组件围绕Element Plus构建，具备良好的交互体验与扩展性。建议在后续迭代中完善缓存策略、优化导出流程与错误恢复能力，持续提升用户体验与系统稳定性。
